...
import {
  PlusIcon, SearchIcon, SlidersHorizontalIcon,
  UsersIcon, XIcon,
} from 'lucide-react'
...
const EmptyState = ({ search }) => (
  ...
  >
    ...
    >
      <UsersIcon size={20} strokeWidth={1.3} style={{ color: 'var(--text-subtle)' }} />
    </div>
    ...
    <p className='text-[12px]' style={{ color: 'var(--text-subtle)' }}>
      {search ? 'Try a different name or position' : 'Add your first employee to get started'}
    </p>
  </div>
)
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8'>
        <div>
          <div className='flex items-center gap-2 mb-2'>
            <UsersIcon size={13} strokeWidth={1.5} style={{ color: 'var(--accent)' }} />
            <span
          ...
          <p className='page-subtitle'>
            {loading ? 'Loading…' : `${filtered.length} of ${employees.length} member${employees.length !== 1 ? 's' : ''}`}
          </p>
        </div>
