...
const EmployeeCard = ({ employee, onDelete, onEdit }) => {
  ...
  return (
    ...
    >
      {/* Accent top strip */}
      <div className='h-[3px] w-full shrink-0' style={{ background: accent.strip, opacity: 0.6 }} />

      {/* Avatar section */}
      <div
        className='relative flex items-center justify-center py-8'
        style={{ background: `radial-gradient(ellipse at center, ${accent.bg} 0%, transparent 70%)` }}
      >
        {/* Avatar */}
        <div
          className='w-16 h-16 rounded-2xl flex items-center justify-center text-xl font-semibold tracking-wide select-none'
          style={{ background: accent.bg, color: accent.text, border: `1px solid ${accent.text}22` }}
        >
          {initials}
        </div>
        ...
        {!employee.isDeleted && (
          <div
            ...
            style={{ background: 'rgba(9,14,24,0.6)', backdropFilter: 'blur(4px)' }}
          >
            <button
        ...
        <div className='flex items-start justify-between gap-2'>
          <div className='min-w-0'>
            ...
            <div className='flex items-center gap-1.5 mt-0.5' style={{ color: 'var(--text-muted)' }}>
              ...
              <p className='text-[11.5px] truncate'>{employee.position || '—'}</p>
            </div>
          </div>
